...
class GetAudio(object):
    ...
    def __init__(self, mix_key=['mix'], source_key='sources'):
        ...
        if not isinstance(mix_key, list):
            raise TransformException(
                f"Expected a list of mix keys"
            )

    def __call__(self, data):

        for key in self.mix_key:
            if key not in data:
                raise TransformException(
                    f"Expected {key} in dictionary "
                    f"passed to this Transform! Got {list(data.keys())}."
                )

        # Check if mix_key is a list of keys
        for key in self.mix_key:
            new_key = 'mix_audio_' + key
            data[new_key] = data[key].audio_data
